...
使用适用于 Azure 的 Cloud Volumes ONTAP 作为存储平台时，请确保 Cloud Manager 具有最新所示的权限 https://occm-sample-policies.s3.amazonaws.com/Policy_for_cloud_Manager_Azure_3.9.12.json["适用于 Azure 的 Cloud Manager 策略"^]。
...
建议卷的大小足以容纳应用程序的总数据集，并能够随着数据集的增长相应地进行扩展。如果工作环境中有多个聚合，请参见 https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-managing-storage.html["管理现有聚合"^] 确定哪个聚合可为新卷提供最多的可用空间。
